...
/*(function ($) {
        ...
        // Lorsqu'une option de catégorie est cliquée
        $('.option.categorie').on('click', function () {
        $('.option.categorie').removeClass('active');
        ...
        // Lorsqu'une option de format est cliquée
        $('.option.format').on('click', function () {
            $('.option.format').removeClass('active');
            $(this).addClass('active');
      ...
      // LOAD MORE AU CHARGEMENT DE LA PAGE
    ...
    function loadMorePhotos() {
        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                if (res.max > 1 && currentPage >= res.max) {
                    $('#load-more').hide();
                } else {
                    $('#load-more').show();
                }
  ...
  function filterPhotos() {
      ...
      $.ajax({
          ...
          success: function (res) {
              ...
              // Réinitialise le bouton 'load-more'
            //  $('#load-more').hide();
          },
          error: function (xhr, status, error) {
              console.error(xhr.responseText);
